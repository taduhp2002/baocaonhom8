...
SELECT COUNT(*) AS 'Số hóa đơn không phải của khách hàng đăng ký thành viên mua'
FROM HOADON H
...
SELECT COUNT(DISTINCT MASP) AS 'Số sản phẩm khác nhau được bán ra trong năm 2006'
FROM CTHD C INNER JOIN HOADON H
ON C.SOHD = H.SOHD
WHERE YEAR(NGHD) = 2006
...
SELECT COUNT(DISTINCT MASP) AS 'Tổng số sản phẩm'
FROM SANPHAM
WHERE NUOCSX = 'TRUNG QUOC'

--33.Tính tổng số sản phẩm của từng nước sản xuất.
SELECT NUOCSX, COUNT(DISTINCT MASP) AS 'Tổng số sản phẩm'
FROM SANPHAM
GROUP BY NUOCSX

--34.Với từng nước sản xuất, tìm giá bán cao nhất, thấp nhất, trung bình của các sản phẩm.
SELECT NUOCSX, MAX(GIA) AS 'Cao nhất', MIN(GIA) AS 'Thấp nhẩt', AVG(GIA) AS 'Trung bình'
FROM SANPHAM
GROUP BY NUOCSX
